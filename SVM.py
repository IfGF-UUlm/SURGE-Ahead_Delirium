...
class DeliriumPredictor():
    ...
    def _train(self):
        """GridSearchCV finds the best possible C for LinearSVC. The LinearSVC using
        this C (i.e. best_estimator_) is used as the base estimator for
        CalibratedClassifierCV, which performs Platt-scaling. The coefficients of
        the LinearSVC are rounded to two decimal places.

        Returns the trained Platt-scaled SVM, which is a
        ...
        est=GridSearchCV(
            ...
            param_grid={'C': np.logspace(-8, 8, 17, base=2)},
            cv=LeaveOneOut(),
            n_jobs=-1,
            verbose=1,
        ).fit(self.X_train, self.y_train).best_estimator_
        est.coef_ = normal_round(est.coef_, 2)
        est.intercept_ = normal_round(est.intercept_, 2)
        ...
        self.svm.calibrated_classifiers_[0].calibrators[0].a_ = (
            normal_round(self.svm.calibrated_classifiers_[0].calibrators[0].a_, 2)
        )
        self.svm.calibrated_classifiers_[0].calibrators[0].b_ = (
            normal_round(self.svm.calibrated_classifiers_[0].calibrators[0].b_, 2)
        )
        return self.svm
    ...
    def predict(self, x, preprocessed=True):
        ...
        return (self.svm.estimator.predict(x), self.svm.predict_proba(x)[:, 1])

    def store_model(self, filename):
        """stores the fitted self.transformer and self.svm as a pickled file"""
        with open(filename, 'wb') as f:
            pickle.dump((self.transformer, self.svm), f)

    @classmethod
    def from_pickle(cls, filename):
        """loads a fitted self.transformer and self.svm from a pickled file"""
        res = cls()
        with open(filename, 'rb') as f:
            res.transformer, res.svm = pickle.load(f)
            return res
